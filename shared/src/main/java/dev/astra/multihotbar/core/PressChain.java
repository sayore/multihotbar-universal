...
/** Loader- and Minecraft-independent multi-press state machine. */
public final class PressChain {
    ...
    public int press(int pressedSlot, long nowNanos, int depth) {
        if (pressedSlot < 0 || pressedSlot > 8) throw new IllegalArgumentException("slot 0..8");
        if (depth < 1) depth = 1;
        boolean chained = pressedSlot == slot
                && lastPress != Long.MIN_VALUE
                && nowNanos >= lastPress
                && nowNanos - lastPress <= windowNanos;
